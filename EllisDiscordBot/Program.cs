...
using EllisDiscordBot.Discord.Handlers;
using EllisDiscordBot.Models.Configuration;
using EllisDiscordBot.Services;
...
public class Program
{
    ...
    // https://discord.com/api/oauth2/authorize?client_id=1057900067287207956&permissions=0&scope=bot%20applications.commands
    // If that link doesn't work, the generators are:
    // https://discord.com/developers/applications/1057900067287207956/oauth2/url-generator
    // or just the bit calculator:
    // https://discord.com/developers/applications/1057900067287207956/bot at the bottom
    ...
    private static void ConfigureAppConfiguration(HostBuilderContext hostContext, IConfigurationBuilder config, string[] args)
    {
        ...
        config.SetBasePath(Directory.GetCurrentDirectory())
            ...
            .AddEnvironmentVariables(prefix: "Ellis_")
            .AddCommandLine(args);
    }
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            serviceCollection.AddHostedService<Worker>()
                .Configure<EventLogSettings>(config =>
                {
                    if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                    {
                        config.LogName = "Ellis";
                        config.SourceName = "Ellis - Discord Bot";
                    }
                });
        }
